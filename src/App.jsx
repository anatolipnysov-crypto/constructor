...
const ROADMAP_STEPS = [
  ...
  { title: 'Предлендинг РСЯ', desc: 'Соберите первый мини-лендинг, подключите кнопки Telegram/MAX, Метрику и пользовательские соглашения.', tab: 'pre' },
...
const PROJECT_FIELDS = [
  ...
  ['prelandingLink', 'Ссылка на предлендинг', 'https://...'],
  ...
  ['metrikaGoal', 'Идентификатор цели подписки', 'subscription_ml_2'],
...
const STEP_INSTRUCTIONS = [
  {
    ...
    tag: 'Правовая база',
    steps: [
      'Откройте инструкцию по регистрации ИП на НПД из материалов проекта.',
      'Подготовьте паспорт, ИНН, СНИЛС и доступ к Госуслугам, если используете подтверждение через них.',
      'Зарегистрируйтесь в нужном сервисе/личном кабинете и выберите режим НПД.',
      'Сохраните реквизиты: ФИО, статус, ИНН, ОГРНИП/ОГРН, email и телефон.',
      'Перенесите эти данные во вкладку "Документы" конструктора.',
      'Проверьте, что реквизиты совпадают с теми, которые будут указаны в политике и согласиях.'
    ],
    note: 'Этот шаг нужен до публикации документов и запуска рекламы, чтобы пользователь видел корректного оператора данных.'
  },
  {
  ...
  },
  {
    ...
    note: 'Предлендинг - первая посадочная после РСЯ. На нём должно быть понятно, что именно человек увидит после клика и какой следующий шаг ему предлагают.'
  },
  {
        ...
        {tab === 'how' && (
          <div className="space-y-4">
            ...
            <div className={`${card} rounded-3xl p-6 shadow-sm border`}>
              ...
              <div className="space-y-2">
                {[
                  ['1. Создайте счётчик', 'В Яндекс.Метрике создайте счётчик на домен предлендинга и сохраните номер счётчика в рабочей панели.'],
                  ['2. Включите аналитику BotHelp', 'В мини-лендинге BotHelp включите "Отслеживать подписку через аналитику" и вставьте номер счётчика.'],
                  ['3. Возьмите идентификатор цели', 'После включения аналитики BotHelp показывает событие подписки. Сохраните идентификатор цели, например subscription_ml_2.'],
                  ['4. Проверьте JavaScript-событие', 'В Метрике цель создаётся как JavaScript-событие; идентификатор в Метрике должен совпадать с тем, что отправляет BotHelp.'],
                  ['5. API-токен для офлайн-конверсий', 'Если передаёте офлайн-конверсии, получите OAuth/API-токен Метрики и используйте его только в защищённых настройках/действиях бота.'],
                  ['6. Действие в боте', 'В нужном шаге BotHelp добавьте действие/запрос, который передаёт достижение цели или офлайн-конверсию по вашей инструкции.']
                ].map((item, i) => (
              ...
              <div className={`${dark ? 'bg-blue-500/10 border-blue-500/30 text-slate-300' : 'bg-blue-50 border-blue-200 text-slate-700'} border rounded-xl p-3 text-xs mt-4`}>
                По официальной логике Метрики цель JavaScript-события достигается вызовом вида <code>ym(XXXXXX, 'reachGoal', 'TARGET_NAME')</code>, где XXXXXX - номер счётчика, а TARGET_NAME - идентификатор цели.
              </div>
            </div>
