...
assert.equal(runtimeScript.includes("reachGoal('registration_started'"), true)
assert.equal(runtimeScript.includes("reachGoal('quiz_start_click')"), false)
assert.equal(runtimeScript.includes("reachGoal('question_answered'"), false)
assert.equal(runtimeScript.includes("reachGoal('registration_click'"), false)
assert.equal(runtimeScript.includes("'quiz_question_'"), false)
...
assert.match(
  runtimeScript,
  /atmospaceReady = true;[\s\S]{0,260}reachGoal\('landing_view'\);[\s\S]{0,120}flushPendingAtmospaceEvents\(\);/,
)
...
assert.equal(publishedHtml.includes('registration_click'), false)
