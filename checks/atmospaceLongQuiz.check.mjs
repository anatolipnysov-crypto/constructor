...
assert.equal(html.includes('class="quiz-registration__button"'), true)
assert.equal(html.includes('data-atmospace-messenger'), false)
assert.equal(html.includes('messenger_button_clicked'), false)
assert.equal(html.includes('r.bothelp.io'), false)
assert.equal(html.includes('Telegram'), false)
assert.equal(html.includes('Открыть в MAX'), false)
assert.equal(html.includes('registration_success'), false)
assert.equal(html.includes('payment_success'), false)
assert.equal(html.includes('registration_click'), false)
assert.equal(html.includes('sessionStorage'), false)
assert.equal(html.includes('localStorage'), false)
assert.doesNotMatch(html, /JSON\.stringify\([^)]*\banswers?\b/i)
assert.doesNotMatch(
  html,
  /(?:href|location|hash|search|URLSearchParams|fetch|payload|body)[\s\S]{0,160}\b(?:quiz_?answers?|selectedOption|optionText|questionText|resultKey|quiz_result)\b/i,
)
assert.doesNotMatch(html, /data-(?:answer|quiz-answer|quiz-result)=/i)
