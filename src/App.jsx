...
const CREATIVE_GUIDE = [
  {
    title: 'Какие материалы нужны до создания креатива',
    items: [
      'Ссылка на предлендинг, куда пойдёт трафик.',
      'Продающая история: из неё берём конфликт, эмоцию, персонажа и формулировки боли.',
      'Оффер: из него берём практикум, цену 990Р, альтернативный вариант участия и смысл результата.',
      'Понимание метода: партнёрская воронка через РСЯ, BotHelp, чат-боты, мини-лендинги и оффер.',
      '3-5 заходов: "устал от курсов", "не получается запустить проект", "нет системы", "не хочу вести блог", "нужна готовая связка".'
    ]
  },
  {
    title: 'Что обязан объяснять креатив',
    items: [
      'Не просто "метод заработка", а что за метод: партнёрская воронка.',
      'Не просто "способ дохода", а через что: РСЯ + предлендинг + бот + продающая история + оффер.',
      'Не просто "без навыков", а почему: человеку дают готовый маршрут запуска, а не предлагают стать экспертом.',
      'Если в объявлении есть обещание "практикум", на посадочной странице тоже должен быть практикум.',
      'Если в заголовке есть "партнёрская воронка", на предлендинге это должно быть видно в тексте.'
    ]
  },
  {
    title: 'Что не писать в холодной рекламе',
    items: [
      'Не писать "лёгкие деньги", "быстрые деньги", "гарантированный доход".',
      'Не показывать банковские карты, платежки, скрины переводов, кредиты и долги.',
      'Не использовать расплывчатые фразы без объяснения: "секретный метод", "новый способ заработка", "схема дохода".',
      'Не обещать конкретную сумму как гарантированный результат.',
      'Не делать креатив, который говорит одно, а предлендинг раскрывает совсем другое.'
    ]
  },
  {
    title: 'Формула креатива, которая понятна модерации',
    items: [
      'Кто: человек, который устал от курсов и хаотичных запусков.',
      'Что: бесплатный/короткий разбор партнёрской воронки.',
      'Как: РСЯ приводит на предлендинг, дальше Telegram/MAX бот ведёт к истории и офферу.',
      'Зачем: увидеть понятный маршрут запуска проекта.',
      'CTA: "Посмотреть разбор", "Узнать как устроена воронка", "Перейти к практикуму".'
    ]
  }
];
...
• 5 заголовков на основе боли ("Устали от курсов?", "Не получается запустить проект?")
• 5 заголовков на основе понятного маршрута ("Разбор готовой воронки", "Пошаговый запуск проекта")
• 5 заголовков на основе разрыва шаблона ("Без блога", "Без сложного маркетинга", "Другой подход")

ВАЖНО ДЛЯ МОДЕРАЦИИ:
• Не использовать прямые обещания заработка в объявлении
• Не писать "гарантированный доход", "быстрые деньги", "без вложений", "5000Р за 2 часа"
• Не упоминать банки, карты, кредиты, платежи и оформление финансовых продуктов в холодной рекламе
• Подача: практикум, разбор метода, пошаговая система, готовая воронка

🔹 ВТОРЫЕ ЗАГОЛОВКИ (15 штук, до 30 символов)
Дополнения к основным заголовкам, усиливают их без финансовых обещаний

🔹 ОПИСАНИЯ (15 штук, до 81 символа каждое)
• Раскрывают выгоду
• Содержат призыв к действию
• Используют цифры и факты
• Не обещают конкретный доход и не давят на финансовое состояние

🔹 БЫСТРЫЕ ССЫЛКИ (8 пар: текст + описание)
Формат:
- Текст ссылки (до 30 символов)
- Описание (до 60 символов)
Темы: "Как это работает", "Программа", "Кому подойдёт", "Отзывы", "Стоимость", "Вопросы", "Видеоразбор", "Начать"

🔹 УТОЧНЕНИЯ (10 штук, до 25 символов)
Короткие спокойные тезисы: "Пошаговый разбор", "Готовая связка", "Без ведения блога", "BotHelp + РСЯ"
...
• Сначала пройти модерацию, потом тестировать более смелые формулировки на отдельных гипотезах
          ...
          <div className="space-y-4">
            <div className={`${card} rounded-3xl p-6 shadow-sm border`}>
              ...
              <div className="space-y-3 mb-6">
                {[
                  ['Открой продающую историю', 'Выпиши 3 боли персонажа: курсы не сработали, нет системы, не хочется вести блог/продавать в лоб.'],
                  ['Открой оффер', 'Выпиши, что человек получает: практикум, разбор готовой связки, маршрут запуска.'],
                  ...
                  ['Собери в Canva', 'Формат 1080x1080, крупный текст, 1 главный смысл, максимум 7-9 слов.'],
            ...
            {/* Формулы креатива */}
            <div className={`${card} rounded-3xl p-6 shadow-sm border`}>
              <h3 className={`text-lg font-black mb-3 ${text}`}>💬 Конкретные формулы для креатива</h3>
              <p className={`text-xs ${textMuted} mb-3`}>Это не шаблоны "вслепую". Берите формулу и подставляйте реальный заход из истории/оффера, чтобы модератор видел конкретный метод.</p>
              <div className="grid md:grid-cols-2 gap-2">
                {[
                  'Разбор: партнёрская воронка через РСЯ и чат-бот',
                  'Как связка РСЯ + BotHelp ведёт человека к офферу',
                  'Не курс, а маршрут запуска: креатив → бот → оффер',
                  'Почему курсы не сработали: нет готовой воронки',
                  'Показываем систему: предлендинг, бот, история, оффер',
                  'Практикум по запуску партнёрской воронки',
                  'Как собрать путь клиента от РСЯ до оффера',
                  'BotHelp + Telegram/MAX: как работает прогрев',
                  'Партнёрская связка без блога и ежедневного контента',
                  'Разбор готовой воронки: что делать по шагам'
                ].map((h, i) => (
