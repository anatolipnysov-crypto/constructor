...
assert.equal((helper.match(/target="_blank"/g) || []).length, 2)
assert.equal((helper.match(/rel="noopener noreferrer"/g) || []).length, 2)
assert.equal((helper.match(/atmospace-messenger-cta/g) || []).length, 2)
assert.match(helper, /data-atmospace-messenger[^\n]*telegram/)
assert.match(helper, /data-atmospace-messenger[^\n]*max/)
